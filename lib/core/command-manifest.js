...
const COMMAND_SIDE_EFFECTS = new Map([
  ...
  ['asset', sideEffect('mixed', {
    mutates_yida: false,
    mutates_local: true,
    read_actions: ['status', 'generate', 'resolve'],
...
const COMMAND_PERMISSIONS = new Map([
  ...
  ...permissionEntries([
    ...
    preauthorized_actions: ['generate', 'resolve --upload-assets', 'resolve --mirror-external'],
  })),
...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('connector.test', ['connector', 'test'], 'connector test --connector-id <id> --action <actionId> [--path-json JSON] [--query-json JSON] [--header-json JSON] [--body-json JSON] [--account-id <id>]', 'help.cmd_connector_test'),
  ...
  },
  {
    ...
    commands: [
      ...
      command('asset', ['asset'], 'asset <status|resolve|generate> [options]', 'help.cmd_asset', {
        requiresLogin: false,
        output: 'text|json',
      }),
